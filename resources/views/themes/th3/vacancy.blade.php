            ...
            <table class="table-striped table-bordered mt-3 table_whoswho">
                <thead class="table-danger">
                    <tr>
                        ...
                        <th width="12%"> {{get_title('archive-date',$langid1)->title??'पुरालेख दिनांक' }}</th>
                ...
                <tbody id="list">
                    @if(count($vacancy) > 0)
                        <?php
                            $count = ($vacancy->currentPage() - 1) * $vacancy->perPage() + 1;
                            foreach($vacancy as $key=>$row):
                        ?>
                            <tr>
                                <td>{{ $count+$key}}</td>
                                <td title="{{$row->title}}"> 
                                ...
                                <td> 
                                    ...
                                    @if(!empty($row->txtuplode))
                                        <a target="_blank" class="link-primary" href="{{ URL::asset('public/upload/admin/cmsfiles/circulars/')}}/{{$row->txtuplode}}" title="{{ URL::asset('public/upload/admin/cmsfiles/circulars/')}}/{{$row->txtuplode}}" > Download
