    ...
    <div class="content-div px-3">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-bordered mt-3 table_whoswho">
                ...
                <tbody id="list">
                    @if(count($consumer_associations) > 0)
                        <?php
                            $count = ($consumer_associations->currentPage() - 1) * $consumer_associations->perPage() + 1;
                            foreach($consumer_associations as $key=>$row):
                        ?>
                            <tr>
                                <td>{{ $count+$key}}</td>
                                <td title="{{$row->title}}"> 
        ...
        {!! $consumer_associations->withQueryString()->links('pagination::bootstrap-5') !!} @php $pageurl = substr(clean_single_input(request()->segment(2)),0,4); @endphp
