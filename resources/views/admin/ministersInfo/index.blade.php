...
@section('content') 
                                    ...
                                    <table class="table table-striped table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                <th>#</th>
                                                <th width="15%">Name</th>
                                                <th width="30%">Minister's Info Type (Category)</th>
                                                <th>Designation </th>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php echo $row->info_position??0; ?> <i id="{{$row->id}}" onclick="editmenu(this);"  class="far editbut fa-edit"></i>
                                                    <span  id="info_position_{{$row->id}}" style="display:none" >
                                                    <input class="w-50" type="number" onchange="savedata(this);" id="{{$row->id}}" name="info_postions" value="" /></span>
    ...
    <script>
        ...
        function savedata(data) {
            ...
            var info_positions = data.value;
            var id = data.id;
        
            var linkurl = "{{ url('/admin/update-whos-who-order')}}";
            jQuery.ajax({
                url: linkurl,
                type: "POST",
                data: {id: id,info_position:info_positions ,update_info_orders:'update_info_orders'},
                cache: false,
                success: function (html) {
                    setTimeout(function(){
                        location.reload();
                    }, 1000); 
                    $("#info_position_"+data.id).hide();
                    $("#success_"+data.id).html('Whos who Position is Updated');
                },
            });
        }
    </script>
